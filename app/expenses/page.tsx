...
import { BUSINESS_AREAS, EXPENSE_CATEGORIES, PAYMENT_METHODS } from '@/lib/constants'
...
export default function ExpensesPage() {
  ...
  }, [expenses, search, filterArea, filterCategory, filterMonth])
  ...
  const handleSave = () => {
    ...
    if (editItem) {
      updateExpense({ ...data, id: editItem.id })
    } else {
      addExpense(data)
    }
  ...
  return (
    <AppLayout>
      <div className="max-w-7xl mx-auto space-y-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">{expenses.length} total records</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {categoryTotals.map(([catId, total], i) => {
            ...
            return (
              <div key={catId} className="rounded-2xl p-4 relative overflow-hidden shadow-md"
                style={{ background: gradients[i % gradients.length] }}>
                <div className="absolute -top-4 -right-4 w-16 h-16 rounded-full opacity-20" style={{ background: 'rgba(255,255,255,0.5)' }} />
                <p className="text-xs font-semibold relative z-10 truncate" style={{ color: 'rgba(255,255,255,0.85)' }}>{cat?.name || catId}</p>
                <p className="text-xl font-extrabold mt-1 text-white relative z-10">{formatCurrency(total)}</p>
              </div>
            )
          })}
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 shadow-[0_1px_3px_0_rgba(0,0,0,0.08)] border border-gray-50">
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <div className="min-w-[160px]">
              <Select
                options={[{ value: '', label: 'All Categories' }, ...EXPENSE_CATEGORIES.map(c => ({ value: c.id, label: c.name }))]}
        ...
        <div className="flex items-center justify-between text-sm text-gray-600">
          ...
          <div className="flex items-center gap-1.5 font-semibold text-[#F97366]">
                ...
                {filtered.length === 0 ? (
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-right font-bold text-[#F97366] whitespace-nowrap">{formatCurrency(item.amount)}</td>
                      <td className="px-5 py-3.5">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editItem ? 'Edit Expense' : 'Add Expense'} size="lg">
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button variant="primary" onClick={handleSave} className="flex-1">{editItem ? 'Save Changes' : 'Add Expense'}</Button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Delete Expense" size="sm">
        <div className="space-y-4">
          <p className="text-sm text-gray-600">Are you sure you want to delete this expense record? This cannot be undone.</p>
